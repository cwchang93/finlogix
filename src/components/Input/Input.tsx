...
const Module = React.forwardRef<HTMLInputElement, IInputProps>((props, ref) => {
  const { label, require, onChangeCallback, onPressEnterCallback, ...others } =
    props; // 先解構
  ...
  return (
    <StyledContainerDiv
      label={props.label}
      className={cx("ld-input", props.className)}
      require={require}
    >
      ...
      <div className={cx("inputWrap", { disabled: props.disabled })}>
        ...
        <StyledInputText
          ...
          onBlur={(e: React.FocusEvent<HTMLInputElement>) => {
            ...
            props.errFunc &&
              props.errType === "onBlur" &&
              setErrMsg(props.errFunc(currentTxt));
          }}
          onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
            ...
            if (props.onChangeValidate)
              isLegal = props.onChangeValidate(currentTxt);
